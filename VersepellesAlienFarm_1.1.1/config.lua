...
-- assign expected values. 					 ~Versepelles
---------------------------------------------------------
...
---------------------------------------------------------
-- This table holds all potential commands issuable to player aliens
-- Probably don't change this, unless adding support within the code as well
AlienFarmCommands = 
{
	["train-capsule"] = {order = "a", command = "train"},
	["goto-capsule"] = {order = "b", command = "goto"},
}

-- Display flying-text above trained units
IndicateTrainedUnits = true
-- How far trained units can be from player before becoming docile
MaxTrainedRange = 100